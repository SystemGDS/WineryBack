...
const getUsers = async (req, res) => {
    ...
    try {
        if(!username){
            ...
            : res.status(400).json({message:`It was not found ${userByUserName}`})
        }
    } catch (error) {
...
const putUser = async (req, res) => {
    ...
    try {
        ...
        : res.status(200).json({message: `No profile found with email ${email}`})
        }
    } catch (error) {
        return res.status(500).json({error: error.message})
    }
...
const banHandler = async(req, res) => {
    ...
    try {
        ...
        userBanned.banned
        ? res.status(200).json({message: `User with email ${email} has been successfully banned`})
        : res.status(200).json({message: `The user has been successfully enabled!`})
    } catch (error) {
        return res.status(500).json({error:error.message})
    }
}
...
const adminHandler = async(req, res) => {
    ...
    try {
        ...
        : res.status(200).json({message:`Administrator status revoked for ${email}`})
    } catch (error) {
        return res.status(500).json({error:error.message})
    }
}
